...
exports.copyObjectMultipart = exports.init = exports.ErrorWithDetails = void 0;
...
function init(client, initialized_logger) {
    ...
    if (!(s3Client instanceof client_s3_1.S3Client)) {
...
function copyObjectMultipart({ source_bucket, object_key, destination_bucket, copied_object_name, object_size, copy_part_size_bytes, copied_object_permissions, expiration_period, server_side_encryption, content_type, content_disposition, content_encoding, content_language, metadata, cache_control, storage_class, }, request_context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return Promise.all(copyPartFunctionsArray)
            .then((copy_results) => {
            logger.info({
                msg: `copied all parts successfully: ${JSON.stringify(copy_results)}`,
...
function copyPart(source_bucket, destination_bucket, part_number, object_key, partition_range, copied_object_name, upload_id) {
    ...
    const command = new client_s3_1.UploadPartCopyCommand(params);
    return s3Client
        .send(command)
        .then((result) => {
        logger.info({
            msg: `CopyPart ${part_number} succeeded: ${JSON.stringify(result)}`,
        });
...
function abortMultipartCopy(destination_bucket, copied_object_name, upload_id, request_context) {
    ...
    return s3Client
        .send(command)
        .then(() => {
        const listCommand = new client_s3_1.ListPartsCommand(params);
        ...
        if (parts_list.Parts && parts_list.Parts.length > 0) {
            const err = new ErrorWithDetails("Abort procedure passed but copy parts were not removed");
...
function completeMultipartCopy(destination_bucket, ETags_array, copied_object_name, upload_id, request_context) {
    ...
    const command = new client_s3_1.CompleteMultipartUploadCommand(params);
    return s3Client
        .send(command)
        .then((result) => {
        logger.info({
